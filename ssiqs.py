"""Source code of sympy's self-initializing quadratic sieve
Available at: https://fossies.org/linux/sympy/sympy/ntheory/qs.py"""
...
def qs(N, prime_bound, M, ERROR_TERM=25, seed=1234):
    ...
    print("Size of Factorbase: ", str(len(factor_base)))
    print("*** Phase 1: Sieving ***")
    while True:
        if ith_poly == 0:
        ...
        pr=len(smooth_relations)
        t=round(tm.time()-start,2)
        frac=(pr+1)/(len(factor_base) + threshold)
        print('\b'*512 +"Relations: "+str(pr)+"/"+str(len(factor_base)+threshold)+" ("+str("%0.2f" % min(100,round(frac*100,2)))+"%)"      
    ...
    print(" ")
    print("Sieving completed in: ", tm.time()-start)
    xlist=[int(smooth_relations[i][0]) for i in range(len(smooth_relations))]
    s_nums=[int(smooth_relations[i][1]) for i in range(len(smooth_relations))]
    factor_base=[factor_base[i].prime for i in range(len(factor_base))]
    
    mstep.matrix_factor(N, xlist, s_nums, factor_base)


if __name__ == "__main__":
    
    qs(3100052103345842701310576241909513955560994164275757719917432041283361,225000,65536*3)